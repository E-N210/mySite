...
//          scale(frameCount%map(mouseX,0,width,1,15))
  ...
  //      line(waveform[0]*2000,waveform[30]*2000,waveform[30]*2000,waveform[15]*2000)
  //
  ...
  for(i=0;i<128;i++){
      ...
      points[i].show=function show(){
        ...
        var hue_adjust =(map(waveform[i],-1,1,-255,255))
        var size_adjust =(map(waveform[i],-0.05,0.05,0,50))

        //QUESTA RIGA DI CODICE è IMPORTANTE
        fill(mouseX/10+hue_adjust,0,hue_adjust*100,spectrum[i])

        fill(mouseX/10+hue_adjust,255,255,spectrum[i])

        ellipse(
           width/2+this.x-waveform[i]*1000,
           height- i*20,
